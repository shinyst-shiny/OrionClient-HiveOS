	...
	internal static class Blake2IvBuilder
	{
		...
		public static ulong[] ConfigB(Blake2BConfig config, Blake2BTreeConfig treeConfig)
		{
			...
			if (config.Salt != null)
			{
				...
				if (config.Salt.Length < 16)
				{
					byte[] paddedSalt = new byte[16];
					config.Salt.CopyTo(paddedSalt, 0);

					rawConfig[4] = Blake2BCore.BytesToUInt64(paddedSalt, 0);
					rawConfig[5] = Blake2BCore.BytesToUInt64(paddedSalt, 8);
				}
				else
				{
